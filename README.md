...
- 上位候補だけをOpenAIのChatGPT系モデルへ送り、Structured Outputsで検証
...
`.env` の `OPENAI_API_KEY` にOpenAI PlatformのAPIキーを設定します。未設定でも定量分析・JSON・HTML生成は動き、ChatGPTによる分析だけを省略します。
...
2. `Settings → Secrets and variables → Actions` に `OPENAI_API_KEY` をRepository secretとして登録します。
...
取得に成功した銘柄・市場指標ごとに `retrieved_at` を保存します。個別再取得した系列は、その再取得の完了時刻を記録します。画面の取得日時・レポート作成日時・AI分析完了日時は日本時間、データ基準日は提供元の市場日付です。株価・出来高とそれらから計算した指標は、同じ系列の取得日時を参照します。

取得時刻を保存していない既存レポートは「未記録」と表示し、作成日時から推測しません。最新ページ・日別ページ・銘柄詳細に、作成時点と「投資は自己判断でお願いします。」を表示します。市場データがリアルタイムではないことも明記します。
